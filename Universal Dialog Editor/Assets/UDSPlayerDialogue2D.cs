...
public class UDSPlayerDialogue2D : MonoBehaviour
{
    ...
    /// one is being picked.
    /// </summary>
    private static List<UDSDialogueStarter2D> dialogueStartersInRange;
    ...
    private void Start()
    {
        ...
        dialogueStartersInRange = new List<UDSDialogueStarter2D>();
    }
    ...
    void Update()
    {
        ...
        {
            UDSDialogueStarter2D dialogueStarter = GetClosestDialogueStarterInRange();
    ...
    public void DialogueStarterEnter(UDSDialogueStarter2D ds)
    {
    ...
    public void DialogueStarterExit(UDSDialogueStarter2D ds)
    {
    ...
    public UDSDialogueStarter2D GetClosestDialogueStarterInRange()
    {
        ...
        return dialogueStartersInRange.Find // Find the Dialogue starter (DS)...
                (ds => dialogueStartersInRange.TrueForAll // ...who is for all other DS in the list...
                    (otherDS => // ... closer to this transform (SqrMagnitude for performance reasons)
                        Vector2.SqrMagnitude((Vector2)ds.transform.position - (Vector2)transform.position)
                        <= Vector2.SqrMagnitude((Vector2)otherDS.transform.position - (Vector2)transform.position)));
    }
}
